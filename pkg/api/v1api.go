...
func NewV1Handler(keystone keystone.Driver, storage storage.Driver) http.Handler {
	...
	r.Methods(http.MethodGet).Path("/query").HandlerFunc(authorize(
		observeDuration(p.Query, "maia_query_duration_seconds", "Duration of a PromQL query for latest values (query)"),
		false,
		"metric:show"))
	r.Methods(http.MethodGet).Path("/query_range").HandlerFunc(authorize(
		observeDuration(
			observeResponseSize(p.QueryRange, "maia_range_query_response_size_bytes", "Size of the results of a PromQL query for a time range (query_range)"),
			"maia_query_range_duration_seconds", "Duration of a PromQL query for latest values (query)"),
		false,
		"metric:show"))
